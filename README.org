...
#+BEGIN_SRC bash
  ...
  #
  # Make sure to change the install path if you don't like the default one
  #
  cmake .. -DCMAKE_INSTALL_INCLUDEDIR=~/my-installed/include -DCMAKE_INSTALL_LIBDIR=~/my-installed/lib
...
**** Use =valgrind= for checking memory leaking, only works for Linux
